import Foundation
// 生成的 tunar_core.swift 直接编译进 App target，类型无需 import。
...
protocol PitchEngineFactory {
    func create(config: TunarConfig) -> PitchEngine
}

// MARK: - UniFFI 实装

final class UniffiPitchEngine: PitchEngine {
    private let inner: TunarEngine
    init(config: TunarConfig) { inner = TunarEngine(config: config) }
    func feed(pcm: [Float]) -> TunarEvent? { inner.feed(pcm: pcm) }
...
struct UniffiFactories {
    ...
    struct PitchFactory: PitchEngineFactory {
        func create(config: TunarConfig) -> PitchEngine { UniffiPitchEngine(config: config) }
    }
...
func defaultTunarConfig(sampleRate: Double = 44100.0) -> TunarConfig {
    TunarConfig(

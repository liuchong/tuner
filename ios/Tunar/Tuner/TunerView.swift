...
struct TunerView: View {
    ...
    @StateObject private var forkVm = TuningForkViewModel(initialConfig: defaultTunarConfig())
    ...
    var body: some View {
        ...
        .onAppear {
            forkVm.refresh(settings.toTunarConfig())
        ...
        .onChange(of: settings.a4Hz) { _, _ in forkVm.refresh(settings.toTunarConfig()) }
        .onChange(of: settings.temperament) { _, _ in forkVm.refresh(settings.toTunarConfig()) }
        ...
        .onChange(of: inTune) { _, newValue in
            if newValue {
                ...
                DispatchQueue.main.asyncAfter(deadline: .now() + 0.5) {
                    if inTune { TunarHaptics.shared.doubleTick() }
                }
            }
        }

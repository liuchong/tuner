...
final class TuningForkViewModel: ObservableObject {
    ...
    private let toneProvider: (TunerConfig) -> [ReferenceTone]
    private let player: ReferenceTonePlaying

    init(
        initialConfig: TunerConfig,
        toneProvider: @escaping (TunerConfig) -> [ReferenceTone] = CorePresets.referenceTones,
    ...
    func refresh(_ config: TunerConfig) {
        player.stop()
        selectedStep = nil
        playingStep = nil

...
interface PitchEngine : AutoCloseable {
    ...
    fun feed(pcm: List<Float>): TunarEvent?
...
fun interface PitchEngineFactory {
    fun create(config: TunarConfig): PitchEngine
}
...
/** UniFFI 绑定之上的对外门面。 */
object TunarCore : TunarCoreApi {
    /** 真实引擎工厂。 */
    val engineFactory = PitchEngineFactory { config ->
        object : PitchEngine {
            private val inner = TunarEngine(config)

            override fun feed(pcm: List<Float>): TunarEvent? = inner.feed(pcm)
    ...
    fun defaultConfig(sampleRate: Double = 44100.0): TunarConfig =
        TunarConfig(
    ...
    fun referenceTones(config: TunarConfig): List<ReferenceTone> {
        val engine = TunarEngine(config)
        return try {

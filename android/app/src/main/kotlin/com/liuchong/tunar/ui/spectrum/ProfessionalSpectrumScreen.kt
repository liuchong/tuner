...
import com.liuchong.tunar.ui.tuner.TunerSignal
import com.liuchong.tunar.ui.tuner.TunerViewModel
...
fun ProfessionalSpectrumScreen(
    ...
    AudioPermissionGate(onGranted = viewModel::startCapture) {
        ...
        LazyColumn(
            ...
            item {
                Column {
                    Text("连续时间图谱", style = TunarTypography.label, color = colors.inkPrimary)
                    Text(
                        "最新声音在顶部 · 约 12 秒历史",
                        style = TunarTypography.caption,
                        color = colors.inkSecondary,
            ...
            item {
                Text("实际峰值", style = TunarTypography.label, color = colors.inkPrimary)
            }

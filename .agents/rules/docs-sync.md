...
项目规格采用简体中文与英文双语维护。下表中的 `docs/*.md` 是中文入口，必须在同一提交中
同步对应的 `docs/en/*.md`；根 README 同步维护 `README.md`（英文）与
`README.zh-CN.md`（简体中文）。`docs/README.md` 是语言索引。
...
若实现时发现 spec 不合理：**先改中英两份 spec 说明理由，再改代码**，禁止代码先行。
